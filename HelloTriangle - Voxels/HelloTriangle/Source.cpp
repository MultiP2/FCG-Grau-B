...
void load()
{
	...
	{
		std::cout << "Erro! Mapa não pode ser carregado." << std::endl;
	}
}
...
void save(string fileName)
{
	...
	for (int j = 0; j < 5; j++) //comprimento
	{
		for (int k = 0; k < 5; k++) //profundidade
		{
			for (int i = 0; i < 5; i++) //largura	
			{
				int iVoxel = voxelmap[i][j][k];
				outputFile << iVoxel << " ";
			}
			outputFile << endl;
		}
